...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        #pruning parameters
        self.prune_start_iter = prune_start_iter
        self.prune_end_iter = prune_end_iter
        self.prune_percent = prune_percent
        self.prune_type = prune_type
        self.resample = resample
        self.comp = np.zeros(self.prune_end_iter, float)
    ...
    def make_mask(self, model):
        ...
        mask = [None]* pruning_step 
        pruning_step = 0
        for name, param in model.named_parameters(): 
            if 'weight' in name:
                tensor = param.data.cpu().numpy()
                mask[pruning_step] = np.ones_like(tensor)
                pruning_step = pruning_step + 1
        pruning_step = 0
            ...
            if not _ite == 0:
                ...
                if reinit:
                    ...
                    for name, param in self.model.named_parameters():
                        if 'weight' in name:
                            weight_dev = param.device
                            param.data = torch.from_numpy(param.data.cpu().numpy() * mask[pruning_step]).to(weight_dev)
                            pruning_step = pruning_step + 1
                    pruning_step = 0
                else:
                    self.original_initialization(mask, initial_state_dict)
            print(f"\n--- Pruning Level [{ITE}:{_ite}/{ITERATION}]: ---")

            # Print Nonzeros table
            comp1 = utils.print_nonzeros(self.model)
            comp[_ite] = comp1
    ...
    def ensure_mask(self):
        ...
        for name, p in self.model.named_parameters():
            if 'weight' in name:
                ...
                tensor *= mask[pruning_step]
                p.data = torch.from_numpy(tensor).to(p.device)
